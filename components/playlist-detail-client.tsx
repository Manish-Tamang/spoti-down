...
import { PlaylistHeader, type PlaylistInfo as PlaylistHeaderInfo } from "@/components/playlist-header";
                ...
                <PlaylistHeader playlist={headerInfo}>
                    <div className="mt-6">
                        ...
                        >
                            ...
                            {isDownloadingAll ? `Downloading (${playlist.tracks.length} tracks)...` : 'Download All'}
                        </Button>
                    </div>
                </PlaylistHeader>
                ...
                <div className="mt-8">
                    ...
                    <div className="space-y-1">
                        {playlist.tracks.map((track, index) => (
                            <TrackCard
                                key={`${track.id}-${index}`} // Combine track.id with index for uniqueness
                                track={{
                                    id: track.id,
                                    title: track.title,
                                    artist: track.artist,
                        ...
                        {playlist.tracks.length === 0 && (
                            <p className="text-zinc-400 text-center py-4">This playlist appears to be empty.</p>
                        )}
                    </div>
                </div>
            </div>
